...
async def show_pending_orders(callback: CallbackQuery):
    ...
    if not orders:
        ...
        await callback.message.edit_text("🕒 Нет заказов ожидающих принятия.", reply_markup=keyboard)
        await callback.answer()
        return
    ...
    await callback.message.edit_text("🕒 Заказы ожидающие принятия:", reply_markup=keyboard)
    await callback.answer()
...
async def show_order_details(callback: CallbackQuery, state: FSMContext):
    ...
    if status == OrderStatus.PENDING.value:
        ...
        keyboard_buttons = [
            [
                InlineKeyboardButton(
                    text="⏸ Перенести в ожидание",
                    callback_data=f"move_to_pending_{order_number}",
                )
            ],
            [
                InlineKeyboardButton(
                    text="✅ Подтвердить заказ", callback_data=f"accept_final_{order_number}"
                )
            ],
        ...
        keyboard_buttons = [
            [
                InlineKeyboardButton(
                    text="⏸ Приостановить работу", callback_data=f"pause_{order_number}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="✅ Заказ выполнен", callback_data=f"complete_{order_number}"
                )
            ],
            [InlineKeyboardButton(text="❌ Отмена заказа", callback_data=f"cancel_{order_number}")],
        ...
        keyboard_buttons = [[InlineKeyboardButton(text="✅ Закончить просмотр", callback_data="menu_history")]]
...
async def accept_final_order(callback: CallbackQuery):
    ...
    await callback.answer(f"✅ Заказ №{order_number} подтверждён (зелёный статус)", show_alert=True)
...
async def show_in_work_orders(callback: CallbackQuery):
    ...
    if not orders:
        ...
        await callback.message.edit_text("🔧 Нет заказов в работе.", reply_markup=keyboard)
        await callback.answer()
        return
    ...
    await callback.message.edit_text("🔧 Заказы в работе:", reply_markup=keyboard)
    await callback.answer()
...
async def show_history_orders(callback: CallbackQuery):
    ...
    if not orders:
        ...
        await callback.message.edit_text("📚 История заказов пуста.", reply_markup=keyboard)
        await callback.answer()
        return
...
async def show_history_orders_message(message: Message):
    ...
    if not orders:
        ...
        await message.answer("📚 История заказов пуста.", reply_markup=keyboard)
        return
